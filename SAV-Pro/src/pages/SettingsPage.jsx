...
import { Bell, DatabaseBackup, Globe2, LogOut, Moon, ShieldCheck, SunMedium, UserRound } from 'lucide-react';
import { Button, Card } from '../components/ui';

const defaultPreferences = { language: 'English', theme: 'light', notifications: true, serviceWindow: '08:00 - 18:00' };
  ...
  return (
    <section className="page-shell settings-page">
      ...
      <div className="settings-grid">
        <Card title="Preferences" icon={Globe2} className="settings-card">
          <div className="settings-form">
            <label className="settings-field"><span>Language</span><select value={preferences.language} onChange={event => updatePreference('language', event.target.value)}><option>English</option><option>French</option></select></label>
        ...
        <Card title="Notifications" icon={Bell} className="settings-card">
          ...
          <div className="settings-info-list"><Info label="SLA alerts" value="Enabled for high priority cases" /><Info label="Daily summary" value="Sent every morning" /><Info label="Planning updates" value="Technician changes and visit reminders" /></div>
        </Card>

        <Card title="Account" icon={UserRound} className="settings-card">
          <div className="settings-profile"><div><strong>{user?.name || 'SAV Pro user'}</strong><span>{user?.role || 'Back office'}</span></div><ShieldCheck size={30} /></div>
          <div className="settings-info-list"><Info label="Workspace" value="SAV Pro" /><Info label="Access level" value={user?.role || 'Operator'} /><Info label="Session" value="HttpOnly cookie session" /></div>
        </Card>

        <Card title="Backend Data" icon={DatabaseBackup} className="settings-card">
          <p className="settings-card-copy">Business data is controlled by the backend databases.</p>
          <Button type="button" variant="primary" onClick={() => notify('Reset must be done through backend seed/migration scripts.', 'error')}>Backend only</Button>
        </Card>
      </div>
    </section>
  );
