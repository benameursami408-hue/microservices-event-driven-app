        ...
        foreach (var message in pending)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (message.RetryCount >= MaxRetries)
                {
                    ...
                    _logger.LogError(
                        ex,
                        "Reclamation outbox message {EventId} ({EventType}) reached the retry limit and is now a logical dead-letter. Inspect OutboxMessages.LastError.",
                        message.Id,
                        message.EventType);
                }
                else
                {
                    _logger.LogWarning(
                        ex,
                        "Failed to dispatch reclamation outbox message {EventId} ({EventType}), retry {RetryCount}/{MaxRetries}",
                        message.Id,
                        message.EventType,
                        message.RetryCount,
                        MaxRetries);
